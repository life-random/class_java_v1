...
public class UserApiHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        try {
            ...
            if (method.equals("GET")) {
                ...
                // 405를 보낼 때는 어떤 메서드가 되는지 Allow 헤더로 알려준 것이 규칙이다
     ...
     * 목록을 복사해서 다시 돌려 준다.
     * 왜 복사하는가?
     * 원본을 그대로 넘기면, Gson이 JSON으로 바꾸면서 순회 도중,
     ...
     * List<User>를 그대로 넘기면 Gson이 JSON 배열로 바꿔 준다.
     */
    ...
    private void handlePost(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            SimpleHttpServer.sendResponse(exchange, 400,
                    SimpleHttpServer.TYPE_TEXT, "JSON 형식 올바르지 않습니다");
            return;
        }

        if ( user == null || user.getName() == null || user.getEmail().isBlank()){
            SimpleHttpServer.sendResponse(exchange, 400,
        ...
        // 4. 저장 처리 id 값은 고정값이 아니라서 저장 결과 를 다시 돌려 준다
